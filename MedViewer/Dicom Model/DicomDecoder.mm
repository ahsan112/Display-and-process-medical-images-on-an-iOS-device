...
@interface DicomDecoder(PrivateMethods)
...
- (id)init {
    ...
    if (self) {
        ...
        dict            = [[DicomDictionary alloc] init];
...
- (void)setDicomFilename:(NSString *)filename {
    ...
    //NSURL *url = [[NSURL alloc]initWithString:dicomFileName];
    //dicomData = [[NSData alloc] initWithContentsOfURL:url];
...
- (NSString *)getString:(NSInteger)length {
    // create buf size of length
    char *buf = (char *)calloc((length + 1), sizeof(char));
    
    NSRange range;
    //assign the location and length withtin the stream of bytes
    range.location = location;
    range.length = length;
    
    //assign the buf the value retrived
    [dicomData getBytes:buf range:range];
    //increase the location to move to next value
    location += length;
    
    //convert the retived value into a string
    ...
    switch (vr) {
        ...
        case UT:
            if ((b[2] == 0) || (b[3] == 0)) {
                ...
                // No vr field in this data element
                vr = IMPLICIT_VR;
...
- (NSString *)getHeaderInfo:(int)tag withValue:(NSString *)inValue {
    ...
    if(tmp) {
        ...
        // substrinf 2 as from the dictionary first 2 chars are the VR type e.g LOPATIENTID
    ...
    // working out the value of elements where no value provided by parameters
    //decding the value fiedl of the element
    
    NSString *value = nil;
    BOOL privateTag = NO;
    
    // determine the Vr data type to retruve the value of the data element
    switch (vr) {
        case FD:
...
- (BOOL)readFileInfo {
    ...
    if (![fileMark isEqualToString:DICM]) {
        // Return indicating that this is not a DICOM file
        //
        dicomFound = NO;
        return NO;
    } else {
        ...
        switch (tag) {
            case TRANSFER_SYNTAX_UID: {
                ...
                if (r1.location != NSNotFound || r2.location != NSNotFound) {
                    compressedImage = YES;
                    // Return indicating that this type of
            ...
            case (WINDOW_CENTER): {
                ...
                unsigned long index = range.location;
                if (index != NSNotFound) {
