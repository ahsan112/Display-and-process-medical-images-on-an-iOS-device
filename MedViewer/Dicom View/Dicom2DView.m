...
@interface Dicom2DView()


@end

@implementation Dicom2DView
...
#pragma mark Create a bitmap on the fly, using 8-bit grayscale pixel data
- (void) createImage8 {
    if (!pix8) {
        return;
    }
...
#pragma mark Create a bitmap on the fly, using 16-bit grayscale pixel data
- (void) createImage16 {
    if (!pix16) {
        return;
    }
...
#pragma mark  Create a RGBA bitmap on the fly, using 8-bit RGB pixel data
- (void) createImage24 {
    if (!pix24) {
        return;
    }
...
#pragma mark set the pixels and create the image
